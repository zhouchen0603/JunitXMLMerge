...
def get_case(testsuite, testcase):
    for case in testsuite._elem._children:
...
def main():
    ...
    if len(files_to_merge) <= 1:
        raise Exception("Can't find XML to merge. Must more than 1 xml file. %s" % str(files_to_merge))
    else:
    ...
    for file in files_to_merge:
        try:
            ...
            continue
        for case in xml._elem._children:
